#!/bin/bash
# Featly Backend — Production Deployment Script
...
echo "=== Featly Backend Deployment ==="
...
sudo -u featly ./venv/bin/alembic upgrade head

# 6. Create uploads directory
echo "[6/7] Setting up directories..."
sudo -u featly mkdir -p "$APP_DIR/backend/uploads"
sudo -u featly mkdir -p "$APP_DIR/backend/logs"

# 7. Install systemd service
echo "[7/7] Installing systemd service..."
cp "$APP_DIR/scripts/featly-backend.service" /etc/systemd/system/
sed -i "s|change-me-in-production|$WS_SECRET|g" /etc/systemd/system/featly-backend.service
systemctl daemon-reload
systemctl enable featly-backend
...
echo ""
echo "Commands:"
echo "  systemctl status featly-backend"
echo "  journalctl -u featly-backend -f"
echo "  systemctl restart featly-backend"
