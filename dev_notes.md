...
- Плагин хранит свою копию cookie в `settings.json` plaintext — ПОКА не шифруется
  (ключ на стороне плагина не предусмотрен; отмечено для v3.2)
- `engine/reconnect.py` — только async-ожидания (CW: не блокировать loop)
- `backend_client._request`: retry (3 × backoff) только для GET — создание не дублируется
- Активные диалоги сохраняются в `data/dialogs_cache.json`, восстанавливаются при включении
...
### Выдача по правилам MM2 (2026-08-16)
- Трейд: до 4 УНИКАЛЬНЫХ предметов в одном окне; количество одного типа — безлимит,
  набирается повторными кликами («Вариант Б», каждый клик = +1 шт)
- Названия предметов: поле заказа (`order.fields`, LotField.name «Назван…»/«Предмет»),
  fallback — title до запятой; чистка от эмодзи/кириллицы (`_clean_item_name` в плагине,
  `_sanitize_search_term` в движке)
- items = [name] × amount (повторы = количество)
- >4 уникальных в заказе → выдача первых 4 с логом остатка; повторные трейды — v3.2
...
- Hub без очередей: `WAIT_FOR_TRADE` и `message_queue` удалены; `REMOVE_WAITLIST` — единственный
  push-сигнал (обновление отмены, чтобы не выдать за отменённый заказ)
- `FORCE_TRADE` / отмена используют `request_waitlist` повторно (самовосстановление)
- Mini-ПК: `engine` настраивается через `FEATLY_WS_URL`/`FEATLY_BOT_ID`/`FEATLY_WS_SECRET`;
  несколько инстансов ↔ уникальные `bot_id`; конфиг профиля `mm2.yaml` — только регионы/тайминги
