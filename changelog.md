...
### Выдача нескольких предметов по правилам MM2-трейда (коммит `…`)
- **Плагин**: `_parse_items` переписан — название берётся из **поля заказа**
  (`order.fields`, `LotField.name` ≈ «Назван…»/«Предмет»/«Товар»), fallback — первый
  сегмент `title`; мусор («Предметы», «Трейд», «шт.») больше не попадает в выдачу;
  количество = повторы названия (Вариант Б)
- **Движок**: лимит трейда MM2 — **до 4 уникальных предметов в одном окне**;
  повторяющиеся названия = количество (каждый клик +1 шт, повторы добавляются отдельно);
  превышение уникальных (>4) — честный лог с перечнем недодобавленных (полная выдача —
  повторный трейд, план v3.2)
- **Поисковый терм санитизируется** (только латиница/цифры): эмодзи/кириллица лота
  больше не ломают ввод в поиск MM2
